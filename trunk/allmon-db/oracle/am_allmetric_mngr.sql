CREATE OR REPLACE PACKAGE am_allmetric_mngr IS
  ...
  PROCEDURE raw_load_to_allmetric_hours(p_i_datetime_start DATE, p_i_datetime_end DATE);
  PROCEDURE raw_load_to_allmetric(p_i_datetime_start DATE, p_i_datetime_end DATE);  
...
CREATE OR REPLACE PACKAGE BODY am_allmetric_mngr IS
  ...
  -------------------------------------------------------------------------------------------------
  -- use after very heavy metrics loading when loading porecedure was switch off for many hours/days
  PROCEDURE raw_load_to_allmetric_hours(p_i_datetime_start DATE, p_i_datetime_end DATE) IS
    -- check necessary dates range
    -- ...
    CURSOR c IS
      --SELECT DISTINCT trunc(arm.ts, 'HH24') AS h
      --FROM   am_raw_metric arm
      --WHERE  arm.ts BETWEEN p_i_datetime_start AND p_i_datetime_end
      --ORDER BY 1;
      SELECT to_date(h, 'YYYY-MM-DD-HH24') AS h, c FROM (
        SELECT to_char(arm.ts, 'YYYY-MM-DD-HH24') AS h, COUNT(*) AS c
        FROM   am_raw_metric arm
        WHERE  arm.ts BETWEEN p_i_datetime_start AND p_i_datetime_end
        GROUP  BY to_char(arm.ts, 'YYYY-MM-DD-HH24'))
      ORDER BY 1;
  BEGIN
    FOR c_rec IN c LOOP
      -- call loading procedures
      dbms_output.put_line('loading raw metrics: ' || to_char(c_rec.h, 'YYYY-MM-DD-HH24') ||' - '|| to_char(c_rec.h + 1/24, 'YYYY-MM-DD-HH24') || ' records: ' || c_rec.c);
  ...
  -- this versoin does not partition the load process
  PROCEDURE raw_load_to_allmetric IS
  BEGIN
    -- MAYBE check resource distribution - potentially if amount of collected metrics is bigger than 100k per hour
    -- ...
    -- call loading procedures
    raw_load_to_allmetric(SYSDATE-1, SYSDATE);
    --COMMIT;
  END;

  -- loads raw metrics data to allmetrics schema
  -- this versoin does not partition the load process
  PROCEDURE raw_load_to_allmetric(p_i_datetime_start DATE, p_i_datetime_end DATE) IS
  BEGIN
    -- MAYBE check resource distribution - potentially if amount of collected metrics is bigger than 100k per hour
    -- ...
    -- call loading procedures
    am_allmetric_mngr.raw_load_dynamicdims(p_i_datetime_start, p_i_datetime_end);
    am_allmetric_mngr.raw_load_metricdata(p_i_datetime_start, p_i_datetime_end);
    --COMMIT;
  END;
  ...
  BEGIN
    ...
    INSERT INTO am_metricsdata(am_met_id, am_rme_id, am_ins_id, am_hst_id, am_rsc_id, am_src_id, am_cal_id, am_tim_id, metricvalue, ts, loadts, observation_id)
      ...
      INNER JOIN am_artifact aa ON (arm.artifactcode = aa.artifactcode AND aa.am_arf_id = ai.am_arf_id)
      INNER JOIN am_host ah ON (arm.hostname = ah.hostname)
      INNER JOIN am_resource ar ON (arm.resourcename = ar.resourcename)
      LEFT OUTER JOIN am_source asr ON (arm.sourcename = asr.sourcename)
      INNER JOIN am_metrictype amt ON (arm.metrictypecode = amt.metriccode AND amt.am_mty_id = ar.am_mty_id)
    ...
    -- this step allows to keep inserting data while the loading process is extracting raw metrics to allmetrics schema
  ...
  -------------------------------------------------------------------------------------------------
  PROCEDURE allmetrics_aggregate(p_i_datetime_start DATE, p_i_datetime_end DATE) IS
  BEGIN
    -- ..
    -- add new resources and sources (regardless soft/sharp filtering is in use) for a new *aggregated* metric type
    -- fill out metricsdata table - using dynamically created sql merge statement
    EXECUTE IMMEDIATE 'SELECT * FROM dual'; -- TODO just a dummy statement tp let compile
  END;
