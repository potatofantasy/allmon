...
select 
  NON EMPTY Crossjoin(Crossjoin({[MODULE_NAME].[ALL].Children}, {[SUBMODULE].[ALL].Children}), {[Measures].[SUM_EXECTIME_MIN]}) ON COLUMNS,
  --NON EMPTY Crossjoin(Crossjoin({[YEAR].[ALL].Children}, {[MONTH].[ALL].Children}), {[DAY].[ALL].Children}) ON ROWS
  NON EMPTY Crossjoin({[YEAR].[ALL].Children}, Crossjoin({[MONTH].[ALL].Children}, {[DAY].[ALL].Children})) ON ROWS
from [MONITOR_ACTION_CLASSES_VMD]

</jp:mondrianQuery>

<c:set var="title01" scope="session">
Monitor data mart (Action classes) - Timeseries 01: Modules and submodules
</c:set>