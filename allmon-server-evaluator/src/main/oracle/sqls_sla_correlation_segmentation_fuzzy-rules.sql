...
from (
 select -- artifactcode, hostname, hostip, instancename, entrypoint, threadname,
       ...
       ceil(10 * (count(*) * sum(r.metricvalue) / 1000)) as sla_value -- 10$ for every started second of an image processing longer by average than 20ms
  from am_raw_metric r
...
-- use temp tab
drop table temp_matching_action ;
...
from (
 select -- calculate coefficients  -- 240 - count of time aggregates in the moving window
       round(action/100), --200*trunc(cpu/200) / 5000 as t, 
       --avg(action) as action_avg, 
       avg(cpu), avg(sla), avg(sla3), avg(dqueue),
...
group by round(action/100) -- 200*trunc(cpu/200) / 5000 
) order by 1 desc;
